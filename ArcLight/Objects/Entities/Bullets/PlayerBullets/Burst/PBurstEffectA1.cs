...
namespace ArcLight
{
    public class PBurstEffectA1 : PlayerBullet
    {

        int frame = 0;
        float expl_time = 30;

        public PBurstEffectA1(Fighter owner, Vector2 pos, float angle) : base(owner, pos, angle)
        {
            ...
            x = pos.X;
            y = pos.Y;
            damage = Status.PBurstEffectA1_Damage;
            is_burst = true;
        ...
        {
            if (!alive) return;
            float scale = 3.0f * (frame / expl_time);
