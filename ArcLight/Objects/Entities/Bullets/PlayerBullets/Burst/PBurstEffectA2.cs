...
namespace ArcLight
{
    public class PBurstEffectA2 : PlayerBullet
    {

        int frame = 0;
        float expl_time = 30;

        public PBurstEffectA2(Fighter owner, Vector2 pos, float angle, bool no_se) : base(owner, pos, angle)
        {
            if (!no_se)
                Audio.PlayBurstExplosionSmallSE();
            this.owner = owner;
            this.angle = angle;
            x = pos.X;
            y = pos.Y;
            damage = Status.PBurstEffectA2_Damage;
            is_burst = true;
        ...
        {
            return Cache.Texture("player_bullet/type_a_burst/bomb_ring", 1);
        }

        protected override Hitbox GetHitbox()
        {
            float scale = 1.5f * (frame / expl_time);
